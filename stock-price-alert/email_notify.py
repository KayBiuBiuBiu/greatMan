...
def send_email_alert(subject: str, content: str) -> bool:
    cfg = _load_mail_cfg()
    if cfg is None:
        return False
    msg = MIMEMultipart()
...
def send_buy_signal_email(subject: str, body: str) -> bool:
    """已配置邮件时发送；未配置则静默跳过。"""
...
def send_sell_signal_email(subject: str, body: str) -> bool:
    """卖出信号邮件；逻辑与买入相同。"""
    if _load_mail_cfg() is None:
        return False
    return send_email_alert(subject, body)
