...
const BUILD_ID = 'dontdoit-repo-v1'
...
function fallbackActions(need) {
  const n = Math.max(need | 0, 2)
  return shuf(FALLBACK_ACTIONS).slice(0, Math.min(FALLBACK_ACTIONS.length, n))
}
  ...
  try {
    ...
    if (body.code === 0 && body.data && body.data.length) {
      ...
      for (let i = 0; i < body.data.length; i += 1) {
        const w = actionFromItem(body.data[i])
        if (w) {
          words.push(w)
        }
      }
      if (words.length >= need) {
        return { words: shuf(words).slice(0, Math.max(need, words.length)), source: 'cloud' }
      }
    }
  } catch (e) {
    console.warn('[dontdoit] generateCharacters fail', e.message || e)
  }
  return { words: fallbackActions(need), source: 'fallback' }
}
...
async function doStartGame(event) {
  ...
  let bank = normalizeBank(event.wordBank || event.actionBank)
  let wordSource = bank.length ? 'client' : ''
  if (!bank.length) {
    const fetched = await fetchActionBank(room.config, n)
    bank = fetched.words || []
    wordSource = fetched.source || 'fallback'
  }
  if (bank.length < n) {
    throw new Error('动作数量不足，需要至少 ' + n + ' 个')
  }
