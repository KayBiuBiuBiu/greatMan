...
async function loadState(gameKind, roomId) {
  ...
  if (k === 'headband') {
    const doc = await db.collection('headband_rooms').doc(rid).get()
    return { kind: 'headband', state: doc.data || null }
  }
  if (k === 'dontdoit') {
    const doc = await db.collection('dontdoit_rooms').doc(rid).get()
    return { kind: 'dontdoit', state: doc.data || null }
  }
  return null
}

function detectPhase(bundle) {
  if (!bundle || !bundle.state) {
    return 'unknown'
  }
  const s = bundle.state
  if (bundle.kind === 'drink') {
    return s.phase || s.status || 'unknown'
  }
  if (bundle.kind === 'undercover') {
    return s.currentPhase || s.phase || 'unknown'
  }
  if (bundle.kind === 'werewolf') {
    return s.currentPhase || s.status || 'unknown'
  }
  if (bundle.kind === 'headband' || bundle.kind === 'dontdoit') {
...
function summarizeForPrompt(bundle) {
  ...
  } else if (bundle.kind === 'headband' || bundle.kind === 'dontdoit') {
    lines.push(`状态: ${s.status}`)
    if (s.winnerNickName) {
      lines.push(`胜者: ${s.winnerNickName}`)
    }
  }
  return lines.join('\n')
}

module.exports = { loadState, summarizeForPrompt, detectPhase }
