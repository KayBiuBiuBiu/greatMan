...
> 云环境 ID：`cloud1-d9g01no7m292bc511-d5e875d`（见 `cloud-env.js`）  
...
|-------|------|
| `waiting` | 等待组长「开始本轮」 |
| `countdown` | 倒计时（**10 秒**，`COUNTDOWN_MS=10000`；揭晓后才响铃/震动） |
...
- 词对由云端调用 `aiPartyService` 生成；AI 不可用时不发词
...
1. `startGame` → 云端调用 `aiPartyService` 生成题目 → 指定本轮画家 → `drawing`
...
- **未购曲库**：小程序内不统一播歌；**组长**固定为当轮主持（`roundHostOpenId`），在其手机上看歌名并**本机外放**，其他人只听、输入歌名抢答
...
1. 确认开发者工具「当前云环境」= `cloud1-d9g01no7m292bc511-d5e875d`。  
