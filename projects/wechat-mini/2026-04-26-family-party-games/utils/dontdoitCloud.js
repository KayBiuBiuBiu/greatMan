...
function formatFail(err) {
  ...
  if (/node-sdk\/ai|wx-server-sdk|Cannot find module/i.test(raw)) {
    return '请重新部署 dontdoitRoomService（云端安装依赖）'
  }
  if (/not\s*found|未部署|FUNCTION_NOT_FOUND/i.test(raw)) {
    return '请部署云函数 dontdoitRoomService'
  }
  return raw.length > 22 ? raw.slice(0, 22) + '…' : raw || '网络异常'
}

function callDontdoit(data, opts) {
  ...
  wx.cloud.callFunction({
    ...
    success: (res) => {
      ...
      console.log('[ddi cloud]', data.action || '?', r.errMsg ? { errMsg: r.errMsg } : r)
    ...
    fail: (err) => {
      /* eslint-disable no-console */
      console.warn('[ddi cloud] fail', data.action || '?', err)
      /* eslint-enable no-console */
      if (!silent) {
