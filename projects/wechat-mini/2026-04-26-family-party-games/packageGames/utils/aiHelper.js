/**
 * 微信云开发 AI：createModel("hunyuan-v3") + model "hy3-preview"（见 cloud-env.js）
 */
...
const {
  ...
  getCloudEnvId,
  debugCloudLog
} = require('../../utils/cloudInit')

const DEFAULT_MODEL = 'hy3-preview'
const DEFAULT_PROVIDER = 'hunyuan-v3'
...
/** @returns {{ provider: string, models: string[] }[]} */
function getClientAiAttempts() {
  try {
    const cfg = require('../../cloud-env.js')
    if (cfg && cfg.aiClientAttempts && cfg.aiClientAttempts.length) {
      return cfg.aiClientAttempts.map((row) => ({
        provider: row.provider || aiProvider,
        models: (row.models && row.models.length ? row.models : modelCandidates).slice()
      }))
    }
  } catch (e) {
    /* ignore */
  }
  return [{ provider: aiProvider, models: getModelCandidates() }]
}
...
function parseGenerateTextRes(res) {
  ...
  const bizCode = (res && res.code) || (body && body.code) || ''
  if (bizCode && String(bizCode) !== '0' && String(bizCode).toUpperCase() !== 'OK') {
    const err = new Error((res && res.message) || (body && body.message) || String(bizCode))
    err.errCode = bizCode
    throw err
  }
  const text = String(
...
function formatAiErr(err) {
  const raw =
    ...
    ''
  if (/EXCEED_TOKEN_QUOTA|TOKEN_QUOTA|429|Too Many Requests/i.test(raw)) {
    return (
      'AI Token 额度已用尽。请到云开发控制台 → AI → 资源包/用量，购买或续费 Token 包后再试（与模型配置无关）。' +
      (raw ? '\n' + String(raw).slice(0, 120) : '')
    )
  }
  if (/AI_MODEL_NOT_FOUND|Model not found|模型未找到|model.*not found/i.test(raw)) {
    return (
      '模型名与控制台不一致。请到云开发 → AI → 文本模型，复制已开通的模型 ID，写入 cloud-env.js 的 aiModels / aiClientAttempts。' +
      (raw ? '\n' + String(raw).slice(0, 120) : '')
    )
  }
...
function clientGenerateText(userPrompt, systemPrompt) {
  ...
  const attempts = getClientAiAttempts()
  let lastErr = null
  let pi = 0
  let mi = 0

  function tryNext() {
    if (pi >= attempts.length) {
      return Promise.reject(lastErr || new Error('客户端 AI 失败'))
    }
    const row = attempts[pi]
    const provider = row.provider || aiProvider
    const models = row.models && row.models.length ? row.models : getModelCandidates()
    if (mi >= models.length) {
      pi += 1
      mi = 0
      return tryNext()
    ...
    const messages = buildChatMessages(userPrompt, systemPrompt, provider)
    return chatModel
      .generateText({
        model: modelName,
        messages: messages
  ...
  generateTextWithRetry('你好', testSystem)
    ...
    .catch((err) => {
      ...
      showAiModal(
        ...
        formatAiErr(err) +
          extra +
          '\n\n请确认：① 云环境 cloud1-d9g01no7m292bc511-d5e875d；② AI 控制台已开通文本模型且 cloud-env.js 中模型名一致；③ 可配置 HUNYUAN_API_KEY 后部署 aiPartyService。'
      )
    })
}
...
function runAi(page, opts) {
  try {
    if (!require('../../data/feature-flags').isAiButtonsEnabled()) {
      return
    }
  } catch (e) {
    /* ignore */
  ...
  if (!wx.cloud) {
    showAiModal('AI 不可用', '请先开通云开发并选择环境 cloud1-d9g01no7m292bc511-d5e875d')
    return
  }
