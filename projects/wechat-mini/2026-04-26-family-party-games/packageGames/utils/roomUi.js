 ...
 */
const { lobbyGuestReadyStats } = require('./roomLobbyReady')
...
function explainHeadbandStartFail(msg, ctx) {
  ...
  if (/仅房主|组长/.test(m)) {
    return { title: '无权限', content: '只有组长可以开始游戏。' }
  }
  return explainGenericStartFail(m, ctx)
}
...
const EXPLAINERS = {
  headband: explainHeadbandStartFail,
...
function buildStartChecks(o) {
  ...
  if (opts.players && opts.isHost !== false) {
    const gr = lobbyGuestReadyStats(opts.players, opts.hostOpenId || '')
    if (gr.guestCount > 0 && !gr.allReady) {
